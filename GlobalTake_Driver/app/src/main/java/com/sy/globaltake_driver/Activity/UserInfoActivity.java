...
public class UserInfoActivity extends AppCompatActivity  implements View.OnClickListener{
    ...
    @Bind(R.id.phone_num)
    TextView phone_num;
    @Bind(R.id.user_carnum)
    TextView user_carnum;
    ...
    private void initData(){
        ...
        String userCarnum = (String) SharePreferenceUtils.getParam(context, Const.driverCarNumber, "");
        user_carnum.setText(userCarnum);
    ...
    public void OnMainEvent(String eventStr){
    ...
    private void changeOtherInfo(String email, final String sex, String userName) {
        ...
        entity.addBodyParameter("type","2");//用户类型1：乘客；2：司机
        if (!sex.isEmpty()){
            entity.addBodyParameter("sex",sex);
        }else {
            entity.addBodyParameter("sex","");
        }
        if (!email.isEmpty()){
            entity.addBodyParameter("email",email);
        }else {
            entity.addBodyParameter("email","");
        }
        if (!userName.isEmpty()){
            entity.addBodyParameter("userName",userName);
        }else {
            entity.addBodyParameter("userName","");
        }
        LanguageUtil.addLanguage(entity);
        x.http().post(entity, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if ("1".equals(jsonObject.getString("code"))) {
                        ...
                        Toast.makeText(UserInfoActivity.this, LanguageUtil.getResText(R.string.toast_dataexception), Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private void clearUserInfo(){
        ...
        SharePreferenceUtils.setParam(context,Const.User_Stauts,Const.carStatus_in);//收车
    }
